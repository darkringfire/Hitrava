...
if sys.version_info < (3, 5, 1):
    sys.stderr.write(
        "You need Python 3.5.1 or later (you are using Python %s.%s.%s).\n"
...
class HiActivity:
    ...
    def __init__(
        ...
        logging.getLogger(PROGRAM_NAME).debug(
            "New HiTrack activity to process <%s>", activity_id
        )
    ...
    @classmethod
    def from_json_pool_swim_data(
        cls, activity_id: str, start: datetime, json_pool_swim_dict
    ):
        ...
        if json_pool_swim_dict is None or len(json_pool_swim_dict) == 0:
            logging.getLogger(PROGRAM_NAME).warning(
                "Swimming activity %s is empty (no segment data) and can not be "
                + "instantiated.",
                activity_id,
            )
            return
    ...
    def from_manual_json_pool_swim_data(
        cls, activity_id: str, start: datetime, duration_millis: int, distance: int
    ):
        swim_activity = cls(activity_id, HiActivity.TYPE_POOL_SWIM)
        swim_activity.start = start
    ...
    def set_activity_type(self, activity_type: str):
        if activity_type in self._ACTIVITY_TYPE_LIST:
            logging.getLogger(PROGRAM_NAME).info(
                "Setting activity type of activity %s to %s",
                self.activity_id,
                activity_type,
            )
            ...
            logging.getLogger(PROGRAM_NAME).error(
                "Invalid activity type <%s>", activity_type
            )
            raise Exception("Invalid activity type <%s>", activity_type)

    def set_pool_length(self, pool_length: int):
        logging.getLogger(PROGRAM_NAME).info(
            "Setting pool length of activity %s to %d", self.activity_id, pool_length
        )
    ...
    def _add_segment_start(self, segment_start: datetime):
        if self._current_segment:
            logging.getLogger(PROGRAM_NAME).error(
                "Request to start segment at %s when there is already a current segment active",
                segment_start,
            )
            return

        logging.getLogger(PROGRAM_NAME).debug(
            "Adding segment start at %s", segment_start
        )
    ...
    def _add_segment_stop(self, segment_stop: datetime, segment_distance: int = -1):
        logging.getLogger(PROGRAM_NAME).debug(
            "Adding segment stop at %s", segment_stop)
        if not self._current_segment:
            logging.getLogger(PROGRAM_NAME).error(
                "Request to stop segment at %s when there is no current segment active",
                segment_stop,
            )
            return
        ...
        self._current_segment["duration"] = int(
            (segment_stop - self._current_segment["start"]).total_seconds()
        )
    ...
    def add_location_data(self, data: List[List[str]]):
        ...
        if (
            ...
            logging.getLogger(PROGRAM_NAME).debug(
                "Found zero location record. Setting activity start to reference %s",
            ...
            location_data["t"] = _convert_hitrack_timestamp(
                location_data["t"], timestamp_ref=self.timestamp_ref
            )
    ...
    def _get_last_location(self) -> Optional[dict]:
        ...
        if self.data_dict:
            reverse_sorted_data = sorted(
                self.data_dict.items(), key=operator.itemgetter(0), reverse=True
            )
    ...
    def _vincenty(self, point1: tuple, point2: tuple) -> float:
        ...
        for iteration in range(MAX_ITERATIONS):
            ...
            logging.getLogger(PROGRAM_NAME).error(
                "Failed to calculate distance between %s and %s", point1, point2
            )
            raise Exception(
                "Failed to calculate distance between %s and %s", point1, point2
            )
    ...
    def add_heart_rate_data(self, data: List[List[str]]):
        ...
        try:
            ...
            if hr_data["hr"] < 1 or hr_data["hr"] > 254:
                logging.getLogger(PROGRAM_NAME).warning(
                    "Invalid heart rate data detected and ignored in data %s", data
                )
        except Exception as e:
            logging.getLogger(PROGRAM_NAME).error(
    ...
    def add_altitude_data(self, data: List[List[str]]):
        ...
        try:
            alti_data = dict(data)
            # Use unique keys. Update keys k -> t and v -> hr
            alti_data["t"] = _convert_hitrack_timestamp(
                float(alti_data.pop("k")))
            alti_data["alti"] = float(alti_data.pop("v"))

            # Ignore invalid altitude data
            if alti_data["alti"] < -1000 or alti_data["alti"] > 10000:
                logging.getLogger(PROGRAM_NAME).warning(
                    "Invalid altitude data detected and ignored in data %s", data
                )
                return
    ...
    # TODO For activities that were tracked using a phone only without a fitness device, there are no s-r records.
    # Hence, in these cases auto detection should use a 'fallback mode' e.g. by using the p-m records
    # (and assume that swimming activities with phone only won't occur)
    def add_step_frequency_data(self, data: List[List[str]]):
        ...
        try:
            ...
            step_freq_data["t"] = _convert_hitrack_timestamp(
                float(step_freq_data.pop("k"))
            )
        ...
        if step_freq_data["s-r"] >= 0:
            ...
            self.activity_params["step frequency data"].append(
                step_freq_data["s-r"])
    ...
    def add_swolf_data(self, data: List[List[str]]):
        ...
        try:
            ...
            if not self.start:
                logging.getLogger(PROGRAM_NAME).warning(
                    "Ignored SWOLF record at relative time <%s> before first "
                    + "record with absolute time.",
                    swolf_data.pop("k"),
                )
                return
            ...
            swolf_data["t"] = self.start + dts_delta(
                seconds=int(swolf_data.pop("k")) + 5
            )
            ...
            # TODO There is a chance that checking on SWOLF only might miss a lap in case two consecutive laps have
            # the same SWOLF (but then again, chances are that stroke and speed data are also identical)
            # TODO Since SWOLF value contains both time and strokes, add extra check to not process consecutive same
            # time laps beyond the SWOLF value.
            if not self._current_segment:
                ...
                if self.last_swolf_data["swf"] != swolf_data["swf"]:
                    ...
                    self._add_segment_start(
                        swolf_data["t"] + dts_delta(microseconds=1))
    ...
    def add_stroke_frequency_data(self, data: List[List[str]]):
        ...
        logging.getLogger(PROGRAM_NAME).debug(
            "Adding stroke frequency swim data %s", data
        )
        ...
        try:
            ...
            if not self.start:
                logging.getLogger(PROGRAM_NAME).warning(
                    "Ignored stroke frequency record at relative time <%s> "
            ...
            # The first record with k=0 is the value registered after 5 seconds of activity.
            stroke_freq_data["t"] = self.start + dts_delta(
                seconds=int(stroke_freq_data.pop("k")) + 5
            )
    ...
    def add_speed_data(self, data: List[List[str]]):
        ...
        try:
            ...
            if not self.start:
                logging.getLogger(PROGRAM_NAME).warning(
                    "Ignored speed record at relative time <%s> before "
                    + "first record with absolute time.",
                    speed_data.pop("k"),
                )
                return
            ...
            # The first record with k=0 is the value registered after 5 seconds of activity.
            speed_data["t"] = self.start + dts_delta(
                seconds=int(speed_data.pop("k")) + 5
            )
    ...
    def fill_missing_data(self):
        ...
        last_sr = None
        last_rs = None
        for _, v in self.data_dict.items():
            if "hr" in v:
                last_hr = v["hr"]
    ...
    def _detect_activity_type(self) -> str:
        ...
        logging.getLogger(PROGRAM_NAME).debug(
            "Detecting activity type for activity %s with parameters %s",
            self.activity_id,
            self.activity_params,
        )
        ...
        if "step frequency min" in self.activity_params:
            ...
            logging.getLogger(PROGRAM_NAME).debug(
                "Activity %s has a calculated average step frequency of %d",
                self.activity_id,
                step_freq_avg,
            )

            if (
                self.activity_params["step frequency min"] == 0
                and self.activity_params["step frequency max"] == 0
            ):
    ...
    def _calc_segments_and_distances(self):
        ...
        logging.getLogger(PROGRAM_NAME).debug(
            "Calculating segment and distance data for activity %s", self.activity_id
        )
        ...
        for _, data in self.data_dict.items():
            if "lat" in data:  # This is a location record
                if last_location is not None:
                    if data["lat"] == 90 and data["lon"] == -80:
                        ...
                        logging.getLogger(PROGRAM_NAME).debug(
                            "Start pause at %s in %s", data["t"], self.activity_id
                        )
                        paused = True
                        self._add_segment_stop(
                            last_location["t"],
                            last_location["distance"] - segment_start_distance,
                        )
                        ...
                        if paused:
                            logging.getLogger(PROGRAM_NAME).debug(
                                "Stop pause at %s in %s", data["t"], self.activity_id
                            )
                            paused = False
                        logging.getLogger(PROGRAM_NAME).debug(
            ...
            # Do not process speed records for open water swim activities
                ...
                "rs" in data and self._activity_type != HiActivity.TYPE_OPEN_WATER_SWIM
            ):
                if last_location is not None:
                    time_delta = data["t"] - last_location["t"]
                    if not paused and (
                        "lat" not in last_location or time_delta > GPS_TIMEOUT
                    ):
    ...
    def get_segment_data(self, segment: dict) -> list:
        ...
        if segment["stop"]:
            ...
            segment_data_dict = {
                k: v for k, v in self.data_dict.items() if segment["start"] <= k
            }

        # Sort data by timestamp (sort on key in data dictionary)
        segment_data = [value for (key, value) in sorted(
            segment_data_dict.items())]
        return segment_data
    ...
    def normalize_distances(self):
        ...
        if (
            self.calculated_distance == 0
            or self.distance == 0
            or self.calculated_distance == self.distance
        ):
            return

        logging.getLogger(PROGRAM_NAME).debug(
            "Normalizing distance data for activity %s", self.activity_id
        )
    ...
    def _calc_pool_swim_data(self) -> list:
        ...
        """
        logging.getLogger(PROGRAM_NAME).info(
            "Calculating swim data for activity %s", self.activity_id
        )
        ...
        for n, segment in enumerate(self._segment_list):
            ...
            raw_data_duration = (
                last_lap_record["t"] - first_lap_record["t"]
            ).total_seconds() + 5
            ...
            lap_data["duration"] = (
                lap_data["swolf"] - lap_data["strokes"]
            )  # Derive lap time from SWOLF - strokes
            if self.pool_length < 1:
                # Pool length not set. Derive estimated distance from raw speed data
                lap_data["speed"] = first_lap_record["rs"] / \
                    10  # estimation in m/s
            ...
            lap_data["stop"] = lap_data["start"] + dts_delta(
                seconds=lap_data["duration"]
            )

            logging.getLogger(PROGRAM_NAME).debug(
                "Calculated swim data for lap %d : %s", n + 1, lap_data
            )
...
class HiTrackFile:
    ...
    def __init__(
        ...
        if not hitrack_filename:
            logging.getLogger(PROGRAM_NAME).error(
                "Parameter HiTrack filename is missing"
            )
        ...
        try:
            ...
            logging.getLogger(PROGRAM_NAME).error(
                "Error opening HiTrack file <%s>\n%s", hitrack_filename, e
            )
            raise Exception("Error opening HiTrack file <%s>",
                            hitrack_filename)
        ...
        try:
            ...
            self.start = _convert_hitrack_timestamp(
                float(os.path.basename(self.hitrack_file.name)[8:18])
            )
        except Exception:
            self.start = None
        ...
        try:
            ...
            self.stop = _convert_hitrack_timestamp(
                float(os.path.basename(self.hitrack_file.name)[20:30])
            )
        except Exception:
            self.stop = None
    ...
    def parse(self) -> HiActivity:
        ...
        if self.activity:
            return (
                self.activity
            )  # No need to parse a second time if the file was already parsed

        logging.getLogger(PROGRAM_NAME).info(
            "Parsing file <%s>", self.hitrack_file.name
        )
        ...
        try:
            ...
            for line_number, line in enumerate(csv_reader, start=1):
                ...
                if (
                    line[0] == "tp=lbs"
                ):  # Location line format: tp=lbs;k=_;lat=_;lon=_;alt=_;t=_
                    # Parse parameters t, lat, lon parameters (alt not parsed)
                    for data_index in [5, 2, 3]:
                        # Parse values after the '=' character
                    ...
                    # Parse parameters k (timestamp) and v (heart rate)
                ...
                # Step frequency (steps/minute) format: tp=s-r;k=_;v=_
                elif line[0] == "tp=s-r":
                    # Parse parameters k (timestamp) and v (step frequency)
                    for data_index in [1, 2]:
                        # Parse values after the '=' character
                        data_list.append(line[data_index].split("="))
                ...
                # Stroke frequency (strokes/minute) format: tp=p-f;k=_;v=_
                elif line[0] == "tp=p-f":
                    # Parse parameters k (timestamp) and v (step frequency)
                    for data_index in [1, 2]:
                        # Parse values after the '=' character
                ...
                # Speed (decimeter/second) format: tp=p-f;k=_;v=_
                elif line[0] == "tp=rs":
                    # Parse parameters k (timestamp) and v (step frequency)
                    for data_index in [1, 2]:
                        # Parse values after the '=' character
            ...
            logging.getLogger(PROGRAM_NAME).error(
                ...
                self.hitrack_file.name,
                line_number,
                line,
                e,
            )
            raise ValueError(f"Error parsing file <{
                             self.hitrack_file.name}> at line <{line_number}>") from e

        finally:
            self._close_file()

        self.activity.sort_data()
        # Add missing data
    ...
    def _close_file(self):
        try:
            if self.hitrack_file and not self.hitrack_file.closed:
                ...
                logging.getLogger(PROGRAM_NAME).debug(
                    "HiTrack file <%s> closed", self.hitrack_file.name)
        except ValueError as e:
            logging.getLogger(PROGRAM_NAME).error(
                "Error closing HiTrack file <%s>\n%s", self.hitrack_file.name, e)
...
class HiTarBall:
    ...
    def __init__(self, tarball_filename: str, extract_dir: str = OUTPUT_DIR):
        ...
        if not tarball_filename:
            logging.getLogger(PROGRAM_NAME).error(
                "Parameter HiHealth tarball filename is missing")

        try:
            self.tarball = tarfile.open(tarball_filename, "r")
        except Exception as e:
            logging.getLogger(PROGRAM_NAME).error(
                "Error opening tarball file <%s>\n%s", tarball_filename, e)
            raise Exception("Error opening tarball file <%s>",
                            tarball_filename)
    ...
    def parse(self, from_date: datetime.date = datetime.date(1970, 1, 1)) -> list:
        try:
            ...
            for tar_info in self.tarball.getmembers():
                if tar_info.path.startswith(self._TAR_HITRACK_DIR) and os.path.basename(tar_info.path).startswith(self._HITRACK_FILE_START):
                    hitrack_filename = os.path.basename(tar_info.path)
                    logging.getLogger(PROGRAM_NAME).info(
                        "Found HiTrack file <%s> in tarball <%s>",
                        hitrack_filename,
                        self.tarball.name,
                    )
                    if from_date:
                        ...
                        hitrack_file_date = _convert_hitrack_timestamp(
                            float(
                                hitrack_filename[len(self._HITRACK_FILE_START):len(
                                    self._HITRACK_FILE_START) + 10]
                            )
                        ).date()
            ...
            logging.getLogger(PROGRAM_NAME).error(
                "Error parsing tarball <%s>\n%s", self.tarball.name, e
            )
            raise ValueError(f"Error parsing tarball <{
                             self.tarball.name}>") from e
    ...
    def _close_tarball(self):
        try:
            if self.tarball:
                self.tarball.close()
                logging.getLogger(PROGRAM_NAME).debug(
                    "Tarball <%s> closed", self.tarball.name
                )
        except ValueError as e:
            logging.getLogger(PROGRAM_NAME).error(
                "Error closing tarball <%s>\n%s", self.tarball.name, e
            )
...
class HiZip:
    ...
    def extract_json(
        ...
        _MOTION_PATH_JSON_FILENAME_ALT = (
            "Motion path detail data & description/motion path detail data.json"
        )
        _WINDOWS_UNZIP_CMD = (
            '7za x -aoa "-o%s" -bb0 -bse0 -bsp2 "-p%s" -sccUTF-8 "%s" -- "%s"'
        )
...
class HiJson:
    ...
    def __init__(
        ...
        if not json_filename:
            logging.getLogger(PROGRAM_NAME).error(
                "Parameter for JSON filename is missing"
            )
    ...
    def parse(self, from_date: datetime.date = datetime.date(1970, 1, 1)) -> list:
        try:
            ...
            json_string = re.sub(r'"partTimeMap":\{(.*?)\},', "", json_string)
            ...
            for n, activity_dict in enumerate(data):
                if "recordDay" in activity_dict:
                    activity_date = dts.strptime(
                        str(activity_dict["recordDay"]), "%Y%m%d").date()
                else:
                    activity_date = dts.fromtimestamp(
                        activity_dict["startTime"] / 1000, tz.utc).date()
                    pass
                ...
                if activity_date >= from_date:
                    ...
                    if "motionPathData" in activity_dict:
                        for motion_path_dict in activity_dict["motionPathData"]:
                            hi_activity = self._parse_activity(
                                motion_path_dict)
                            if hi_activity:
                                self.hi_activity_list.append(hi_activity)
                    else:
            ...
            if n == -1:
                logging.getLogger(PROGRAM_NAME).info(
                    "No activities found to convert in JSON file <%s>",
                    self.json_file.name,
                )
            return self.hi_activity_list
        except Exception as e:
            ...
            raise Exception("Error parsing JSON file <%s>",
                            self.json_file.name)
    ...
    def _parse_activity(self, activity_dict: dict) -> Optional[HiActivity]:
        ...
        hitrack_data = re.sub(r"HW_EXT_TRACK_DETAIL@is", "", hitrack_data)
        hitrack_data = re.sub(
            r"\&\&HW_EXT_TRACK_SIMPLIFY@is(.*)", "", hitrack_data, flags=re.DOTALL
        )

        # Get additional activity detail data
        activity_detail_data = activity_dict["attribute"]
        activity_detail_data = re.sub(
            r"HW_EXT_TRACK_DETAIL@is(.*)\&\&HW_EXT_TRACK_SIMPLIFY@is",
            "",
        ...
        time_zone = tz(
            dts_delta(hours=time_zone_hours_offset,
                      minutes=time_zone_minutes_offset)
        )

        # Get start date and time in UTC
        activity_start = dts.fromtimestamp(
            activity_dict["startTime"] / 1000, tz.utc)
        ...
        if sport_type in self._UNSUPPORTED_JSON_SPORT_TYPES:
            logging.getLogger(PROGRAM_NAME).warning(
                "Activity from %s has an unsupported "
        ...
        if any(activity_dict["sportType"] in i for i in self._JSON_SPORT_TYPES):
            sport = [
                item[1] for item in self._JSON_SPORT_TYPES if item[0] == sport_type
            ][0]

        # Save HiTrack data to HiTrack file
        hitrack_filename = "%s/HiTrack_%s_%s" % (
            self.output_dir,
            _get_tz_aware_datetime(
                activity_start, time_zone).strftime("%Y%m%d_%H%M%S"),
            sport,
        )
        ...
        if self.export_json_data:
            ...
            logging.getLogger(PROGRAM_NAME).info(
                "Exporting JSON data of activity from %s to file %s",
                activity_start,
                json_filename,
            )
            try:
                with open(json_filename, "w+") as json_file:
                    # Encapsulate the exported JSON data in a list
                    json_file.write("[")
                ...
                try:
                    ...
                    logging.getLogger(PROGRAM_NAME).error(
                        "Error closing JSON export file <%s>\n%s", json_filename, e
                    )

        logging.getLogger(PROGRAM_NAME).info(
            "Saving activity from %s to HiTrack file %s for parsing",
            activity_start,
            hitrack_filename,
        )
        try:
        ...
        if sport == HiActivity.TYPE_POOL_SWIM:
            ...
            if "mSwimSegments" in activity_detail_dict:
                ...
                if sport_data_source == self._SPORT_DATA_SOURCE_MANUAL:
                    logging.getLogger(PROGRAM_NAME).info(
                        "Swimming activity %s has been manually added, conversion "
                        "will only contain basic activity data.",
                        activity_id,
                    )
                    ...
                    logging.getLogger(PROGRAM_NAME).warning(
                        "Swimming activity %s with sport data source %d has no "
                        "swim segment data and can not be converted.",
                        activity_id,
                        sport_data_source,
                    )
                    return
            ...
            if hi_activity is None:
                logging.getLogger(PROGRAM_NAME).warning(
                    "Swimming activity %s has empty swim segment data and can "
                    + "not be converted.",
                    activity_id,
                )
                return
            ...
            timestamp_ref = datetime.datetime(
                year=activity_start.year,
                month=activity_start.month,
                day=activity_start.day,
            )

            hitrack_file = HiTrackFile(
                hitrack_filename,
                timestamp_ref=timestamp_ref,
                start_timestamp_ref=activity_start,
            )
            hi_activity = hitrack_file.parse()
            if sport != HiActivity.TYPE_UNKNOWN:
                ...
                logging.getLogger(PROGRAM_NAME).warning(
                    "Activity %s from %s has an unknown activity "
        ...
        hi_activity.stop = activity_start + dts_delta(
            milliseconds=activity_dict["totalTime"]
        )
    ...
    def _close_json(self):
        try:
            if self.json_file and not self.json_file.closed:
                ...
                logging.getLogger(PROGRAM_NAME).debug(
                    "JSON file <%s> closed", self.json_file.name
                )
        except Exception as e:
            logging.getLogger(PROGRAM_NAME).error(
                "Error closing JSON file <%s>\n%s", self.json_file.name, e
            )
...
class TcxActivity:
    ...
    _STRAVA_SPORT_TYPES = [
        ...
        (
            ...
            "running",
            # Not recognized by Strava TCX upload, change activity type after upload manually to Virtual Run.
        ),
        (
            HiActivity.TYPE_INDOOR_CYCLE,
            "biking",
            # Not recognized by Strava TCX upload, change activity type after upload manually to Virtual Ride.
        ),
        (
            HiActivity.TYPE_CROSS_TRAINER,
            "elliptical",
            # Not recognized by Strava TCX upload, change activity type after upload manually to Elliptical.
        ),
        (HiActivity.TYPE_OTHER, _SPORT_OTHER),
        (
            HiActivity.TYPE_CROSSFIT,
            "crossfit",
            # Not recognized by Strava TCX upload, change activity type after upload manually to Crossfit.
        ),
        (HiActivity.TYPE_UNKNOWN, _SPORT_OTHER),
        (HiActivity.TYPE_CROSS_COUNTRY_RUN, "running"),
    ]
    ...
    def __init__(
        ...
        insert_altitude: bool = False,
    ):
        if not hi_activity:
            logging.getLogger(PROGRAM_NAME).error(
                "No valid HiTrack activity specified to construct TCX activity."
            )
            raise Exception(
                "No valid HiTrack activity specified to construct TCX activity."
            )
    ...
    def _get_sport(self):
        ...
        try:
            if self.tcx_xml_schema:
                ...
                sport = [
                    item[1]
                    for item in self._TCX_SPORT_TYPES
                    if item[0] == self.hi_activity.get_activity_type()
                ][0]
            else:
                # Use Strava sport types when XML validation is NOT used
                sport = [
                    item[1]
                    for item in self._STRAVA_SPORT_TYPES
                    if item[0] == self.hi_activity.get_activity_type()
                ][0]
        finally:
            if sport == "":
                logging.getLogger(PROGRAM_NAME).warning(
                    "Activity <%s> has an undetermined/unknown sport type.",
    ...
    def generate_xml(self) -> xml_et.Element:
        ...
        logging.getLogger(PROGRAM_NAME).debug(
            "Generating TCX XML data for activity %s", self.hi_activity.activity_id
        )
        try:
            # * TrainingCenterDatabase
            training_center_database = xml_et.Element("TrainingCenterDatabase")
            training_center_database.set(
                "xsi:schemaLocation",
            ...
            # ** Activities
            el_activities = xml_et.SubElement(
                training_center_database, "Activities")
            ...
            el_id.text = _get_tz_aware_datetime(
                self.hi_activity.start).isoformat("T")
            # Generate the activity xml content based on the type of activity
            if self.hi_activity.get_activity_type() in [
                HiActivity.TYPE_WALK,
                HiActivity.TYPE_RUN,
            ...
            elif self.hi_activity.get_activity_type() in [
                ...
                logging.getLogger(PROGRAM_NAME).warning(
                    "Activity %s of type %s has no explicit XML generation method. "
            ...
            el_lang_id = xml_et.SubElement(
                el_author, "LangID"
            )  # TODO verify if required/correct
            el_lang_id.text = "en"
            el_part_number = xml_et.SubElement(
                el_author, "PartNumber"
            )  # TODO verify if required/correct
            el_part_number.text = "000-00000-00"
            el_name.text = "H%ctr%cv%c" % (chr(105), chr(97), chr(97))
        except Exception as e:
            logging.getLogger(PROGRAM_NAME).error(
        ...
        for n, segment in enumerate(self.hi_activity.get_segments()):
            ...
            if segment_data:
                ...
                for data in segment_data:
                    ...
                    el_time.text = _get_tz_aware_datetime(
                        data["t"]).isoformat("T")

                    if "lat" in data:
                        el_position = xml_et.SubElement(
                            el_trackpoint, "Position")
                        el_latitude_degrees = xml_et.SubElement(
                            el_position, "LatitudeDegrees"
                        )
                        el_latitude_degrees.text = str(data["lat"])
                        el_longitude_degrees = xml_et.SubElement(
                            el_position, "LongitudeDegrees"
                        )
                        el_longitude_degrees.text = str(data["lon"])

                    if "alti" in data:
                        el_altitude_meters = xml_et.SubElement(
                            el_trackpoint, "AltitudeMeters"
                        )
                    ...
                    if "distance" in data:
                        el_distance_meters = xml_et.SubElement(
                            el_trackpoint, "DistanceMeters"
                        )
                        el_distance_meters.text = str(data["distance"])

                    if "hr" in data:
                        el_heart_rate_bpm = xml_et.SubElement(
                            el_trackpoint, "HeartRateBpm"
                        )
                    ...
                    # Step frequency (for walking and running)
                    if "s-r" in data:
                        if self.hi_activity.get_activity_type() in (
                            HiActivity.TYPE_CYCLE,
                            HiActivity.TYPE_INDOOR_CYCLE,
                        ):
                            el_cadence = xml_et.SubElement(
                                el_trackpoint, "Cadence")
                            el_cadence.text = str(int(data["s-r"] / 2))
                    if "rs" in data or "s-r" in data:
                        el_extensions = xml_et.SubElement(
                            el_trackpoint, "Extensions")
                        el_tpx = xml_et.SubElement(el_extensions, "ns3:TPX")
                        if "rs" in data:
                            el_speed = xml_et.SubElement(el_tpx, "ns3:Speed")
                            el_speed.text = str(data["rs"])
                        if "s-r" in data and self.hi_activity.get_activity_type() in (
                            HiActivity.TYPE_WALK,
                            HiActivity.TYPE_RUN,
                            HiActivity.TYPE_INDOOR_RUN,
                            HiActivity.TYPE_HIKE,
                            ...
                            el_run_cadence = xml_et.SubElement(
                                el_tpx, "ns3:RunCadence")
    ...
    def _generate_swim_xml_data(self, el_activity):
        ...
        for n, lap in enumerate(self.hi_activity.get_swim_data()):
            ...
            el_time.text = _get_tz_aware_datetime(lap["start"]).isoformat("T")
            el_distance_meters = xml_et.SubElement(
                el_trackpoint, "DistanceMeters")
            el_distance_meters.text = str(cumulative_distance)

            # Add track points for heart rate, position and distance
            for i, lap_detail_data in enumerate(
                self.hi_activity.get_segment_data(
                    self.hi_activity.get_segments()[n])
            ):
                if (
                    "hr" in lap_detail_data
                    or "lat" in lap_detail_data
                    or "distance" in lap_detail_data
                ):
                    el_trackpoint = xml_et.SubElement(el_track, "Trackpoint")
                    el_time = xml_et.SubElement(el_trackpoint, "Time")
                    # TODO Check --utc
            ...
            el_time.text = _get_tz_aware_datetime(lap["stop"]).isoformat("T")
            el_distance_meters = xml_et.SubElement(
                el_trackpoint, "DistanceMeters")
            el_distance_meters.text = str(cumulative_distance)
        return
    ...
    def _generate_lap_header_xml_data(self, el_activity, segment) -> xml_et.Element:
        ...
        el_lap.set("StartTime", _get_tz_aware_datetime(
            segment["start"]).isoformat("T"))
        ...
        if self.hi_activity.calories > 0:
            if self.hi_activity.calculated_distance > 0 and segment["distance"] > 0:
                segment_calories = round(
                    self.hi_activity.calories
                    * segment["distance"]
                    / self.hi_activity.calculated_distance
                )
            else:
                total_duration = (
                    self.hi_activity.stop - self.hi_activity.start
                ).seconds
                segment_calories = round(
                    self.hi_activity.calories *
        ...
        el_intensity = xml_et.SubElement(
            el_lap, "Intensity"
        )  # TODO verify if required/correct
        el_intensity.text = "Active"
        el_trigger_method = xml_et.SubElement(
            el_lap, "TriggerMethod"
        )  # TODO verify if required/correct
    ...
    def save(self, tcx_filename: str = None):
        ...
        try:
            logging.getLogger(PROGRAM_NAME).info(
                "Saving TCX file <%s> for HiTrack activity <%s>",
                self.tcx_filename,
                self.hi_activity.activity_id,
            )
            self._format_xml(self.training_center_database)
            xml_element_tree = xml_et.ElementTree(
                self.training_center_database)
            # If output directory doesn't exist, make it.
            ...
            with open(self.tcx_filename, "wb") as tcx_file:
                tcx_file.write(
                    '<?xml version="1.0" encoding="UTF-8"?>'.encode("utf8"))
                xml_element_tree.write(tcx_file, "utf-8")
        except Exception as e:
            logging.getLogger(PROGRAM_NAME).error(
                "Error saving TCX file <%s> for HiTrack activity <%s>\n%s",
                self.tcx_filename,
                self.hi_activity.activity_id,
                e,
            )
            ...
            try:
                if tcx_file and not tcx_file.closed:
                    tcx_file.close()
                    logging.getLogger(PROGRAM_NAME).debug(
                        "TCX file <%s> closed", tcx_file.name
                    )
            except Exception as e:
                logging.getLogger(PROGRAM_NAME).error(
                    "Error closing TCX file <%s>\n%s", tcx_file.name, e
                )
    ...
    def _validate_xml(self, tcx_xml_filename: str):
        ...
        logging.getLogger(PROGRAM_NAME).info(
            "Validating generated TCX XML file <%s> for activity <%s>",
            tcx_xml_filename,
            self.hi_activity.activity_id,
        )
        ...
        try:
            ...
            logging.getLogger(PROGRAM_NAME).error(
                "Error validating TCX XML for activity <%s>\n%s",
                self.hi_activity.activity_id,
                e,
            )
            raise Exception(
...
def _init_tcx_xml_schema():
    ...
    with tempfile.TemporaryDirectory(PROGRAM_NAME) as tempdir:
        ...
        try:
            logging.getLogger(PROGRAM_NAME).info(
                "Retrieving TCX XSD from the internet. Please wait."
            )
        ...
        try:
            ...
            logging.getLogger(PROGRAM_NAME).warning(
                "TCX XML XSD schema was successfully retrieved from the web "
                + "but could not be initialized. Validation will not be performed. "
                + "The following exception occured: %s",
                e,
            )


def _convert_hitrack_timestamp(
    hitrack_timestamp: float, timestamp_ref: datetime = None
) -> datetime:
    ...
    if (
        timestamp_ref is not None and hitrack_timestamp < 604800
    ):  # Assume activity not longer than a week.
        # Relative seconds from timestamp reference
        return dts.fromtimestamp(
            (timestamp_ref + dts_delta(seconds=hitrack_timestamp)).timestamp(), tz.utc
        )
    elif timestamp_digits == 9:
        # Absolute epoch timestamp
        return dts.fromtimestamp(int(hitrack_timestamp), tz.utc)

    divisor = (
        10 ** (timestamp_digits - 9)
        if timestamp_digits > 9
        else 0.1 ** (9 - timestamp_digits)
    )
    return dts.fromtimestamp(int(hitrack_timestamp / divisor), tz.utc)


def _get_tz_aware_datetime(naive_datetime: dts, time_zone: tz = None):
...
def _init_argument_parser() -> argparse.ArgumentParser:
    ...
    def from_date_type(arg):
        try:
            ...
            msg = "Invalid date or date format (expected YYYY-MM-DD): '{0}'.".format(
                arg
            )
    ...
    def pool_length_type(arg):
        ...
        if len < 1:
            raise argparse.ArgumentTypeError(
                "Pool length must be an positive integer value."
            )
        if len == 1013:
    ...
    output_group.add_argument(
        "--validate_xml",
        help="Validate generated TCX XML file(s). NOTE: requires xmlschema library \
...
def main():
    ...
    logging.getLogger(PROGRAM_NAME).info(
        "Running on Python version %s.%s.%s",
        sys.version_info[0],
        sys.version_info[1],
        sys.version_info[2],
    )
    ...
    if args.file:
        ...
        if args.zip:
            json_filename = HiZip.extract_json(
                args.zip, args.output_dir, args.password)
        elif args.json and zipfile.is_zipfile(args.json):
            json_filename = HiZip.extract_json(
                args.json, args.output_dir, args.password
            )
        else:
